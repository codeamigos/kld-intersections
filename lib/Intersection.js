 ...
 *  @returns {Intersection}
 */
Intersection.intersectCirclePolygon = function(c, r, _polygon) {
  var polygon = closePolygon(_polygon)
  var result = new Intersection('No Intersection')
  ...
  if (result.status !== 'Intersection') {
    ...
    for (var i = 0; i <= 360; i++) {
      ...
      circlePoints.push(new Point2D(x, y))
    }

    if (circlePoints.every(point => Intersection.includesPolygonPoint(polygon, point))) {
...
Intersection.intersectLinePolygon = function(a1, a2, _points) {
  ...
  if (result.status !== 'Intersection' && Intersection.includesPolygonLine(points, a1, a2)) {
    result.status = 'Inside'
  }
 ...
 *  @returns {Intersection}
 */
Intersection.intersectPolygonPolyline = function(_polygon, polyline) {
  var polygon = closePolygon(_polygon)
  var result = new Intersection('No Intersection')
  var length = polygon.length
  ...
  for (var i = 0; i < length - 1; i++) {
    var a1 = polygon[i]
    var a2 = polygon[i + 1]
    var inter = Intersection.intersectLinePolyline(a1, a2, polyline)
    result.appendPoints(inter.points)
  }
  ...
  if (result.status !== 'Intersection' && Intersection.includesPolygonPolyline(polygon, polyline))
    result.status = 'Inside'
 ...
 *
 *  @param {Array<Point2D>} polygon
 *  @param {Array<Point2D>} polygon2
 *  @returns {boolean}
 */
Intersection.includesPolygonPolygon = function(poligon, poligon2) {
  return Intersection.includesPolygonPolyline(poligon, poligon2)
}
